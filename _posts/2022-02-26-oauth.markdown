...
应用是资源的使用者，系统是资源提供者，用户是资源所有者。 
现实生活中， 应用和系统各自有一个实例， 用户有多个实例。 应用和系统之间通过OAuth2协议通信。 用户在通信过程中参与（授权）。


按照参考资料1中的比喻， 应用是快递公司，系统是小区，用户是小区业主。 OAuth2的过程是业主授权快递公司在一定的时间内可以报口令自由出入小区。每个快递员在进入小区时候，小区会在对应的业主许可下，给这个快递员分配口令。口令在一定时间内有效。在同一时间，会存在很多个不同业主授权的有效口令，但一个业主在同一时间只有一个有效口令。 角色间的类比关系如下。

|-----|--------|-----|--------|
|应用 | 下图中的Client| GitHub OAuth APP | 快递公司 | 
| 系统 | 下图中的Authorization Server和Resource Server |GitHub | 小区 | 
| 用户 | 下图中的Resource Owner| GitHub用户 | 小区业主 | 


![picture 4](/img/1646056955927.png)  
